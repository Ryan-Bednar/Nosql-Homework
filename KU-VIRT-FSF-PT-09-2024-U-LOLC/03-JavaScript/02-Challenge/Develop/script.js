...
const collectEmployees = function () {
  ...
  do{
    ...
    const newEmployee = employee(firstName, lastName, salary);
    employeesArray.push(newEmployee);

  }while (window.confirm(`Add another employee?`));
...
employeesArray= employeesArray.concat(currentEmployees);
...
const displayAverageSalary = function (employeesArray) {
  ...
  if(!employeesArray || employeesArray.length === 0){
    console.log("No employees have been entered");
    return 0;
  }
  let sum= 0;
...
const getRandomEmployee = function (employeesArray) {
  ...
  if (employeesArray.length === 0){
    console.log("No employees available selection.");
    return;
  }

  const index= Math.floor(Math.random()*employeesArray.length);
  const winner = employeesArray[index];
